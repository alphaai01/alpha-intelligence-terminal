    ...
    <div className="bg-[#0a1628] border-b border-white/[0.06] px-4 py-2 flex items-center justify-between">
      ...
      <div className="flex items-center gap-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="hidden sm:block">
            <div className="text-[10px] text-white/40 tracking-[0.2em] font-mono uppercase">
        ...
        <div className="flex items-center gap-3 pl-4 border-l border-white/[0.08]">
          ...
          <div>
            <div className="text-[10px] text-white/40 tracking-wider font-mono">
              GLOBAL TENSION INDEX (GTI)
            </div>
            <div className="flex items-center gap-2">
              <span className={`text-xl font-bold font-mono ${getGtiColor()}`}>
                {gtiValue}
              </span>
              <span className="text-amber-400 text-xs font-mono">{gtiChange}</span>
              <span
                className={`text-[10px] px-2 py-0.5 rounded border font-semibold tracking-wider ${getGtiLabelColor()}`}
              >
                {getGtiLabel()}
              </span>
            </div>
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-full border border-green-500/20 bg-green-500/5">
          ...
          <span className="text-white/40 text-xs font-mono">· {feedCount} feeds</span>
        </div>
        ...
        <div className="flex items-center gap-2 text-white/50">
          ...
          <span className="text-xs font-mono">{currentTime || "00:00:00"} UTC</span>
        </div>
      </div>
    </div>
  )
